...
object JuntoRunner extends Logging {
  ...
  def apply (graph: Graph, mu1: Double = 1.0, mu2: Double = .01, mu3: Double = .01, 
    ...
    apply("mad", graph, maxIters, mu1, mu2, mu3, Integer.MAX_VALUE, 
          false, verbose, new ArrayList[Map[String,Double]], null)
  }
  ...
  def apply (algo: String, graph: Graph, maxIters: Int, 
             ...
             initialEstimatedScores: TraversableOnce[Label]) {
    ...
    if (useActors) {
      ...
      val propagator = algo match {
        ...
        new ModifiedAdsorption(graph, keepTopKLabels, mu1, mu2, mu3)
          .setInitialEstimatedScores(initialEstimatedScores)
...
object JuntoConfigRunner extends Logging {
  ...
  def apply (config: Hashtable[String,String], 
    ...
    //modified: read initial estimated scores of nodes
    val initialEstimatedScores = initialScoresConfigLoader(config)
    ...
    JuntoRunner(algo, graph, maxIters, mu1, mu2, mu3, keepTopKLabels,
                useBipartiteOptimization, verbose, resultList, initialEstimatedScores)
